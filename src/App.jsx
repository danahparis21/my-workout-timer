...
function App() {
  ...
  const minutesHandlers = useSwipeable({
    ...
    onSwiping: (event) => {
      minutesRef.current.style.transform = `translateY(${
        event.deltaY * 0.3
      }px)`;
    },
    onSwiped: () => {
  ...
  const secondsHandlers = useSwipeable({
    ...
    onSwiping: (event) => {
      secondsRef.current.style.transform = `translateY(${
        event.deltaY * 0.3
      }px)`;
    },
    onSwiped: () => {
    ...
    >
      {!timerStarted && (
        <div className="w-full max-w-lg flex flex-col items-center justify-center">
          ...
          {/* Timer display - BIG and beautiful */}
          <div className="flex items-center justify-center gap-[3vw] text-[32vw] sm:text-[20vw] font-bold text-ovalBg w-full cursor-pointer leading-none mb-2">
            {editing ? (
              <>
                <input
                  type="tel"
              ...
              <>
                ...
                >
                  ...
                  <div className="text-[3vw] sm:text-[2vw] text-pink-400 mt-2">
                    MIN
                  </div>
                </div>
          ...
          <div className="text-pink-400 text-sm mb-4">
            {editing
              ? "Type numbers, tap outside to save"
              : "Swipe ↑↓ to adjust • Tap to type"}
          </div>

          {/* Quick preset buttons - Compact but not too small */}
          <div className="flex flex-wrap justify-center gap-2 mb-6 max-w-sm">
            {[30, 60, 90, 120, 180, 300].map((seconds) => (
              <button
                key={seconds}
                onClick={() => setTime(seconds)}
                className="px-3 py-1.5 border border-pink-300 text-pink-500 rounded-full text-sm hover:bg-pink-50 active:bg-pink-100 transition-all duration-200"
              >
                {seconds < 60
                  ? `${seconds}s`
                  : seconds === 60
          ...
          <button
            onClick={handleStart}
            className="border border-rosePink text-rosePink text-lg sm:text-xl font-poppins px-6 py-2 rounded-full transition-all duration-300 hover:bg-rosePink/10 active:scale-95 active:bg-rosePink/20"
          >
            Start
          </button>
        </div>
